...
export default function Pushed() {
  ...
  const museumName = museumAddresses.map(item => item.original_Name);
  const address = museumAddresses.map(item => item.formatted)
    const listName = [];

    for (let i=0; i < museumAddresses.length; i++) {
        listName.push(
